...
mw.loader.using(["mediawiki.util", "mediawiki.Uri"], function () {
	...
	window.fireworkShow = (new function () { // jshint ignore:line
		var defaults = {
			...
			targetElement: null, // element which sparks will be appended to (null = document.body) - can be an element 
			// ID eg. "myDiv", or a DOM node reference
			useFadeEffect: true, // When recycling fallen sparks (or rarely, when falling), have it "melt" and fade out 
			// if browser supports it
			usePositionFixed: true, // true = sparks does not shift vertically when scrolling. May increase CPU load, 
			// disabled by default - if enabled, used only where supported
			usePixelPosition: false, // Whether to use pixel values for sparks top/left vs. percentages. Auto-enabled if 
